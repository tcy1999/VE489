...
const char* flag_str[] = {"SYN", "FIN", "DATA", "ACK", "NACK"};
struct buf {
    unsigned int seq;    // seq of the packet in it
    char* packet;
    unsigned int isNAKed; // 0 if the data buffered in this slot has not been NAKed, 1 if has been NAKed
};
...
int sender(const char *receiver_ip, int receiver_port, int myport, int window_size, const char *filename, const char *log_file) {
    ...
    // SYN
    srand(time(NULL));
    int random_seq = rand() + (length/MAX_PAYLOAD_SIZE + 2);
    ...
    while(1) {
        ...
        if (n > 0) {
            ...
            if (recv_syn.flag == 3 && recv_syn.seq == syn.seq) {
                break;
            }
        }
        gettimeofday(&stop, NULL);
    ...
    while(1){
        ...
        if (n > 0) {
            ...
            if (ack.flag == 4){
                // NACK
                struct SRHeader header;
                if (window[0].seq <= ack.seq && ack.seq <= window[window_size-1].seq) {
                    memcpy(&header, window[ack.seq - window[0].seq].packet, sizeof(struct SRHeader));
                    sendto(sockfd, window[ack.seq - window[0].seq].packet, MAX_PACKET_SIZE, 0,
                ...
                if (ack.seq <= (window[0].seq + window_size)) {
                    ...
                    for (int i = window_size - diff; i < window_size; i++) {
                        if (window[i].seq <= length / MAX_PAYLOAD_SIZE) {
                            ...
                            sendto(sockfd, chunks[window[i].seq], MAX_PACKET_SIZE, 0,
                                   (const struct sockaddr *) &receiver_addr,
                                   sizeof(struct sockaddr_in));
                            if (input_log(log_file, header) == -1) {
                                printf("Open send_logfile failed!\n");
        ...
        gettimeofday(&stop, NULL);
        if ((double) (stop.tv_sec - start.tv_sec) * 1000 + (double) (stop.tv_usec - start.tv_usec) / 1000 > 400) {
            // timeout to deal with ACK/NAK loss, but this time only retransmits packet at the beginning of the window
            struct SRHeader header;
            memcpy(&header, window[0].packet, sizeof(struct SRHeader));
            sendto(sockfd, window[0].packet, MAX_PACKET_SIZE, 0, (const struct sockaddr *) &receiver_addr,
                   sizeof(struct sockaddr_in));
            if (input_log(log_file, header) == -1) {
                printf("Open send_logfile failed!\n");
                return -1;
        ...
        if (n > 0) {
            ...
            if (header.flag == 2) {
                ...
                if (header.seq < window[0].seq) {
                    ...
                    // window[0].seq <= header.seq < window_size + window[0].seq,  receiving packet
                    struct SRHeader ack = {3, 0, 0, 0};
                    if (header.seq > window[0].seq) {
                        ack.seq = window[0].seq;
                    ...
                    if (header.seq == window[0].seq) {
                        ...
                        for (int j = i; j < window_size; j++) {
                            ...
                            memset(window[j].packet, 0, MAX_PACKET_SIZE);
                            window[j-i].isNAKed = window[j].isNAKed;
                            window[j].isNAKed = 0;
                            window[j].seq += i;
                        }
                    ...
                    int gap_index = -1; // index of first gap in the window that has not been NAKed
                    char* zero_mem = malloc(MAX_PACKET_SIZE);
                    memset(zero_mem, 0, MAX_PACKET_SIZE);
                    for (int i = 0; i < (window_size-1); i++){
                        if (memcmp(zero_mem, window[i].packet, MAX_PACKET_SIZE) == 0 && !window[i].isNAKed) {
                            for (int j = i+1; j < window_size; j++) {
                                if (memcmp(zero_mem, window[j].packet, MAX_PACKET_SIZE) != 0) {
                                    gap_index = i;
                                    break;
                                }
                            }
                    ...
                    free(zero_mem);
                    if (gap_index >= 0 && gap_index < window_size){
                        // nack
                        ack.flag = 4;
                        ack.seq = window[gap_index].seq;
                        window[gap_index].isNAKed = 1;
                    }
                    sendto(sockfd, (const struct SRHeader *) &ack, sizeof(struct SRHeader), 0, (const struct sockaddr *) &si_other,
                           sizeof(struct sockaddr_in));
                    if (input_log(log_file, ack) == -1) {
